...
class BatchInsertProcessor(BaseProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        stmt_text = f"INSERT INTO {self._table} ({columns}) VALUES ({placeholders})"  # noqa: S608 — self._table провалидирован _validate_table (regex), columns провалидированы _validate_identifier
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        async with bundle.async_session_maker() as session:
            for cols, params_list in groups.items():
                ...
                stmt_text = (
                    f"UPDATE {self._table} SET {set_clause} "  # noqa: S608 — self._table/_key_field провалидированы _validate_table/_validate_identifier
...
class BatchDeleteProcessor(BaseProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        async with bundle.async_session_maker() as session:
            ...
            stmt_text = f"DELETE FROM {self._table} WHERE {self._key_field} = ANY(:ids)"  # noqa: S608 — self._table/_key_field провалидированы _validate_table/_validate_identifier
