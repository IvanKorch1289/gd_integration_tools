...
class TestNotebookExecuteProcessor:
    ...
    async def test_notebook_execute_cells(self) -> None:
        ...
        exchange.set_property("notebook_cells", [
            {"cell_type": "code", "source": "print(1+1)"},
        ])

        mock_outputs = [{"cell_index": 0, "outputs": [{"output_type": "stream", "text": "2\n"}]}]

        with patch.object(proc._svc, "execute_notebook", new_callable=AsyncMock) as mock_execute:
    ...
    async def test_notebook_execute_failure(self) -> None:
        ...
        exchange = _make_exchange()
        exchange.set_property("notebook_cells", [{"cell_type": "code", "source": "raise"}])
...
class TestNotebookExportProcessor:
    ...
    async def test_notebook_export_html(self) -> None:
        ...
        with patch.object(proc._svc, "export_notebook", new_callable=AsyncMock) as mock_export:
