"""Shared JSON utilities — moved from ``dsl/codec/json.py`` (Cycle 31 P1).

Эти утилиты (json_dumps/loads, dumps_bytes/str, canonical_json_bytes, ...)
не содержат DSL-логики — это чистый orjson-wrapper с поддержкой Pydantic/
dataclasses/UUID/datetime/Decimal/Enum/bytes. Размещение в ``core/codec/``
(вместо ``dsl/codec/``) разрешает 13 layer-violation: infrastructure модули
теперь импортируют из ``core.*`` вместо ``dsl.*``.

Backward-compat: ``dsl/codec/json.py`` остаётся re-export shim.
"""
