"""Render logic для ``13_Cron_Builder`` (S144 W3 extraction).

Visual builder для cron-выражений + live preview ``Next 5 executions``
+ timezone-aware (Europe/Moscow default) + dry-run simulation +
...
Извлечено из ``13_Cron_Builder.py`` (149 LOC → split):
...
def render() -> None:
    """Top-level entry-point, вызывается из thin ``13_Cron_Builder.py``.
