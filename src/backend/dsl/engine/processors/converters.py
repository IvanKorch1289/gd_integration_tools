...
_STRATEGIES: dict[str, ConversionStrategy] = {'json→yaml': JsonToYaml(), 'yaml→json': YamlToJson(), 'json→msgpack': JsonToMsgpack(), 'msgpack→json': MsgpackToJson(), 'json→xml': JsonToXml(), 'xml→json': XmlToJson(), 'dict→xml': JsonToXml(), 'xml→dict': XmlToJson(), 'csv→parquet': CsvToParquet(), 'parquet→csv': ParquetToCsv(), 'html→json': HtmlToJson(), 'json→bson': JsonToBson(), 'bson→json': BsonToJson(), 'dict→csv': DictToCsv(), 'json→csv': DictToCsv(), 'csv→dict': CsvToDict(), 'csv→json': CsvToDict(), 'dict→yaml': JsonToYaml(), 'yaml→dict': YamlToJson()}
