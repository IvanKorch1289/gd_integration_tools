...
async def test_system_info(service: AdminService) -> None:
    ...
    with (
        patch("src.backend.services.core.admin.route_registry", mock_route),
        patch("src.backend.services.core.admin.action_handler_registry", mock_action),
        patch("src.backend.services.core.admin.disabled_feature_flags", {"f1"}),
        patch("src.backend.services.core.admin._list_services", return_value=["svc"]),
    ):
