"""Unit-тесты ``services.ai.prompts`` — coverage ratchet (Post-Plan A Sprint 20).

core/ai/prompts service package facade: re-exports 3 symbols
(LangfusePromptStorage class + PromptEntry dataclass + get_prompt_storage
singleton getter). ~5 stmts, 0% coverage.
...
class TestPromptsFacadeAllExports:
    ...
    def test_module_docstring_present(self) -> None:
        """Module docstring описывает prompt-storage (Langfuse adapter)."""
...
class TestPromptsFacadeIdentity:
    ...
    def test_get_prompt_storage_is_callable(self) -> None:
        """``get_prompt_storage`` — callable (singleton getter)."""
