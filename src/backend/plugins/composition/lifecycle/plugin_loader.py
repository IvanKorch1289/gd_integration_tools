...
* ``_shutdown_plugin_loaders`` — 25 LOC
...
async def bootstrap_v11_plugin_loader(app: FastAPI) -> None:
    """R1.fin (ADR-042/044) — поднять PluginLoader под feature-flag.
...
async def shutdown_plugin_loaders(app: FastAPI) -> None:
    """R1.fin — обратный порядок: сначала RouteLoader, затем PluginLoader."""
...
async def handle_v11_changes(app: FastAPI, changes: set) -> None:
    ...
    Логика:
    * Любое изменение ``plugin.toml`` → re-discover (PluginLoader
