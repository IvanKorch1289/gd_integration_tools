...
class ClickHouseAuditService:
    ...
    async def _get_client(self) -> Any:
        ...
        with self._lock:
            ...
            host = getattr(settings.clickhouse, "host", "localhost") if hasattr(settings, "clickhouse") else "localhost"
            port = getattr(settings.clickhouse, "port", 8123) if hasattr(settings, "clickhouse") else 8123
            database = getattr(settings.clickhouse, "database", "default") if hasattr(settings, "clickhouse") else "default"
    ...
    async def emit(self, event: AuditEvent) -> None:
        ...
        try:
            ...
            _logger.debug("audit emit ok: event_type=%s event_id=%s", event.event_type, event.event_id)
    ...
    async def emit_batch(self, events: list[AuditEvent]) -> None:
        ...
        if not feature_flags.audit_clickhouse_enabled:
            _logger.debug(
                "audit_clickhouse_enabled=False, skip emit_batch count=%d",
                len(events),
            )
            return
