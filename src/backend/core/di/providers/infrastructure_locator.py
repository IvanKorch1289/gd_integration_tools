...
if TYPE_CHECKING:
    # S3.5: imports нужны для narrowed return types top-10 most-used providers
    # (ClickHouseClient / ElasticSearchClient / MongoDBClient / ObjectStorage /
    # RedisClient / EventBusFacade). Lazy runtime imports в registry entries
    ...
    #
    # S3.5: top-10 most-used providers (по usage-grep, см.
    # tests/unit/core/di/providers/test_top10_providers_typing.py) получают
    # narrowed return types вместо ``Callable[[], Any]``. Это устраняет
    # ``Any``-leak на самых частых import-сайтах (8+ в src/ + tests/),
    # сохраняя lazy runtime semantics. Динамические ``get_X_attr`` оставлены
    # ``Callable[[str], Any]`` — narrow для них невозможен by design.
    ...
    get_clickhouse_client_class: Callable[[], type[ClickHouseClient]]  # S3.5: narrowed
    ...
    get_elasticsearch_client_class: Callable[
    ...
    get_event_bus_facade_provider: Callable[[], EventBusFacade]  # S3.5: narrowed
    ...
    get_mongodb_client_class: Callable[[], type[MongoDBClient]]  # S3.5: narrowed
    get_object_storage_class: Callable[[], type[ObjectStorage]]  # S3.5: narrowed
    get_order_event_class: Callable[[], Any]
    get_pipeline_event_class: Callable[[], Any]
    get_prompt_cache_middleware: Callable[[], Any]
    get_correlation_id: Callable[[], str]  # S3.5: narrowed
    get_redis_client_class: Callable[[], type[RedisClient]]  # S3.5: narrowed
    get_redis_client_factory: Callable[[], Callable[[], RedisClient]]  # S3.5: narrowed
    ...
    get_unified_rate_limiter_attr: Callable[[str], Any]  # S3.5: dynamic by design
    get_dsl_variables_attr: Callable[[str], Any]  # S3.5: dynamic by design
...
def _load_provider(module_path: str, attr: str) -> Any:
    ...
    return getattr(_importlib.import_module(f"src.backend.{module_path}"), attr)
...
_PROVIDERS_REGISTRY: dict[str, tuple[str, str]] = {
    "abstract_repository_class": (
        "infrastructure.repositories.base",
        "AbstractRepository",
    ),
    "clickhouse_client_class": (
        "infrastructure.clients.storage.clickhouse",
        "ClickHouseClient",
    ),
    "dsl_step_executor_class": ("infrastructure.workflow.executor", "DSLStepExecutor"),
    "durable_workflow_processor_class": (
    ...
    "elasticsearch_client_class": (
        "infrastructure.clients.storage.elasticsearch",
        "ElasticSearchClient",
    ),
    "env_aes_gcm_key_provider_class": (
        "infrastructure.security.token_registry",
        "EnvAESGCMKeyProvider",
    ),
    "env_aesgcm_key_provider_class": (
        "infrastructure.security.token_registry",
        "EnvAESGCMKeyProvider",
    ),
    "event_bus_factory": (
    ...
    "event_schema_validation_error_class": (
        "infrastructure.clients.messaging.event_bus",
        "EventSchemaValidationError",
    ),
    "external_db_registry": (
        "infrastructure.database.database.accessors",
        "get_external_db_registry",
    ),
    "external_db_registry_factory": (
        "infrastructure.database.database.accessors",
        "get_external_db_registry",
    ),
    "db_initializer_factory": (
        "infrastructure.database.session_manager",
        "get_db_initializer",
    ),
    ...
    "inject_openai_prompt_cache": (
        "infrastructure.ai.prompt_cache_middleware",
        "inject_openai_prompt_cache",
    ),
    "inject_openai_prompt_cache_factory": (
        "infrastructure.ai.prompt_cache_middleware",
        "inject_openai_prompt_cache",
    ),
    "inject_prompt_cache": (
    ...
    "inject_prompt_cache_factory": (
        "infrastructure.ai.prompt_cache_middleware",
        "inject_prompt_cache",
    ),
    "jupyter_hub_client_class": (
        "infrastructure.clients.external.jupyter_hub",
        "JupyterHubClient",
    ),
    "jupyter_hub_error_class": (
        "infrastructure.clients.external.jupyter_hub",
        "JupyterHubError",
    ),
    "jupyter_hub_server_class": (
        "infrastructure.clients.external.jupyter_hub",
        "JupyterHubServer",
    ),
    "jupyter_hub_user_class": (
        "infrastructure.clients.external.jupyter_hub",
        "JupyterHubUser",
    ),
    "kafka_producer_class": ("infrastructure.messaging", "kafka_pool_registration"),
    "main_session_manager_factory": (
        "infrastructure.database.session_manager",
        "main_session_manager",
    ),
    "main_session_manager_getter": (
        "infrastructure.database.session_manager",
    ...
    "redis_client_factory": (
        "infrastructure.clients.storage.redis",
        "get_redis_client",
    ),
    "redis_token_registry_class": (
        "infrastructure.security.token_registry",
        "RedisTokenRegistry",
    ),
    "repository_for_model_factory": (
        "infrastructure.repositories.base",
        "get_repository_for_model",
    ),
    "route_event_class": ("infrastructure.clients.messaging.event_bus", "RouteEvent"),
    "scheduler_manager_class": (
        "infrastructure.scheduler.scheduler_manager",
        "SchedulerManager",
    ),
    "scheduler_manager_factory": (
        "infrastructure.scheduler.scheduler_manager",
        "get_scheduler_manager",
    ),
    "sqlalchemy_repository_class": (
        "infrastructure.repositories.base",
        "SQLAlchemyRepository",
    ),
    "three_tier_rag_cache_class": (
        "infrastructure.cache.rag.three_tier",
        "ThreeTierRagCache",
    ),
    "caching_decorator_class": (
        "infrastructure.decorators.caching.decorator",
        "CachingDecorator",
    ),
