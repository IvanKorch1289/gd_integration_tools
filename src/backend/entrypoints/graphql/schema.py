...
async def _dispatch_action(action: str, payload: dict[str, Any] | None=None) -> ActionResult:
    ...
    try:
        result = await _unified_dispatch(action=action, payload=payload, source='graphql')
        data = result
        ...
        return ActionResult(action=action, success=False, error=f"Action '{action}' не зарегистрирован")
...
def _schema_to_order(data: Any) -> OrderType:
    ...
    return OrderType(id=d.get('id', 0), pledge_gd_id=d.get('pledge_gd_id'), pledge_cadastral_number=d.get('pledge_cadastral_number'), order_kind_id=d.get('order_kind_id'), order_kind=order_kind, is_active=d.get('is_active', True), is_send_to_gd=d.get('is_send_to_gd', False), errors=d.get('errors'), response_data=d.get('response_data'), object_uuid=str(d['object_uuid']) if d.get('object_uuid') else None, email_for_answer=d.get('email_for_answer'), created_at=d.get('created_at'), updated_at=d.get('updated_at'), files=files)
...
def _schema_to_user(data: Any) -> UserType:
    ...
    if isinstance(data, dict):
        ...
        d = {'id': 0, 'username': ''}
    return UserType(id=d.get('id', 0), username=d.get('username', ''), email=d.get('email'), is_superuser=d.get('is_superuser', False), is_active=d.get('is_active', True), created_at=d.get('created_at'), updated_at=d.get('updated_at'))
...
def _schema_to_order_kind(data: Any) -> OrderKindType:
    ...
    if isinstance(data, dict):
        ...
        d = {'id': 0}
    return OrderKindType(id=d.get('id', 0), name=d.get('name'), description=d.get('description'), skb_uuid=d.get('skb_uuid'), created_at=d.get('created_at'), updated_at=d.get('updated_at'))

async def _dispatch_dsl(route_id: str, payload: dict[str, Any]) -> DslResult:
    """Диспетчеризует вызов через DslService."""
    try:
        dsl = get_dsl_service()
        exchange = await dsl.dispatch(route_id=route_id, body=payload, headers={'x-source': 'graphql'})
        result_body = exchange.out_message.body if exchange.out_message else None
        return DslResult(route_id=route_id, status=exchange.status.value, result=result_body, error=exchange.error)
    except KeyError:
        return DslResult(route_id=route_id, status='failed', error=f"Маршрут '{route_id}' не зарегистрирован")
    except Exception as exc:
...
class Query:
    ...
    async def file(self, file_id: int) -> FileType | None:
        ...
        if result.success and result.data and isinstance(result.data, dict):
            return FileType(**{k: result.data.get(k) for k in ('id', 'name', 'object_uuid', 'created_at', 'updated_at') if k in result.data})
        return None
...
class Mutation:
    ...
    async def send_email(self, to_emails: list[str], subject: str, message: str) -> ActionResult:
        """Выполнить операцию send email."""
        return await _dispatch_action('tech.send_email', {'to_emails': to_emails, 'subject': subject, 'message': message})
    ...
    async def dsl_execute(self, route_id: str, payload: JSON | None=None) -> DslResult:
...
class Subscription:
    ...
    async def route_trace(self, route_id: str, info: Info) -> AsyncGenerator[TraceEventType]:
        ...
        async for event in tracer.subscribe(route_id):
            yield TraceEventType(route_id=event.route_id, processor_name=event.processor_name, processor_type=event.processor_type, phase=event.phase, duration_ms=event.duration_ms, timestamp=event.timestamp, error=event.error)
    ...
    async def system_health(self, info: Info) -> AsyncGenerator[SystemEventType]:
        ...
        while True:
            try:
                ...
                yield SystemEventType(event_type='health_check', data=result, timestamp=datetime.now(UTC).isoformat())
            except Exception as exc:
                yield SystemEventType(event_type='health_check_error', data={'error': str(exc)}, timestamp=datetime.now(UTC).isoformat())
