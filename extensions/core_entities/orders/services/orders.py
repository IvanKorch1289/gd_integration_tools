...
Полная бизнес-логика (CRUD + интеграция с СКБ-Техно + S3 + ES)
перенесена из shim-модуля.
"""
        ...
        async with self._service_error_boundary():
            ...
            if has_pdf_result:
                await self.get_order_result(
                    order_id=order_id,
                    response_type=type("ResponseTypeChoices", (), {"value": "PDF"})(),
                )

            update_data: dict[str, Any] = {
                "response_data": get(json_result, "response.data", {})
            }

            message = get(json_result, "response.data.Message", "Не готов")
