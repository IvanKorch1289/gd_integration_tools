...
class AvroDecodeProcessor(BaseProcessor):
    ...
    def __init__(self, schema: dict[str, Any] | None=None, *, name: str | None=None) -> None:
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        reader = fastavro.reader(buf, reader_schema=self._schema) if self._schema is not None else fastavro.reader(buf)
        records = list(reader)
...
def _resolve_protobuf_class(message_class: str) -> Any:
    ...
    * ``"package.module.ClassName"`` (legacy fallback).
    ...
    if ':' in message_class:
        ...
        if not module_name:
            raise ValueError(f"protobuf message_class must be 'module:Class' or 'module.Class', got: {message_class!r}")
        ...
        for item in items:
            ...
            for sub_key, sub_value in item.items():
                if isinstance(sub_value, dict):
                    raise ValueError(f"TOML encoder: nested dict внутри array-of-tables '{name}' не поддерживается")
    ...
    class _MdParser(HTMLParser):
        ...
        def handle_endtag(self, tag: str) -> None:
            if self._stack and self._stack[-1] == tag:
                self._stack.pop()
            mapping_close = {'strong': '**', 'b': '**', 'em': '*', 'i': '*', 'code': '`', 'pre': '\n```\n', 'p': '\n'}
...
class JsonLinesDecodeProcessor(BaseProcessor):
    ...
    def __init__(self, *, ignore_blank_lines: bool=True, name: str | None=None) -> None:
