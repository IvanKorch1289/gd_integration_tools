...
class RouteBuilder(
    ...
    # dispatch_action / invoke — перенесены в dsl.builders.integration.IntegrationMixin
    # (Stage 2.5).
    ...
    # auth — перенесён в dsl.builders.integration.IntegrationMixin (Stage 2.5).
    ...
    # choice / do_try / retry — перенесены в dsl.builders.control_flow.ControlFlowMixin
    # (Stage 2.4).

    # to_route — перенесён в dsl.builders.integration.IntegrationMixin (Stage 2.5).

    # parallel / saga / dead_letter / idempotent / fallback — перенесены
    # в dsl.builders.control_flow.ControlFlowMixin (Stage 2.4).
    ...
    # throttle / delay — перенесены в dsl.builders.control_flow.ControlFlowMixin
    # (Stage 2.4).
    ...
    # circuit_breaker / loop / timeout — перенесены в
    # dsl.builders.control_flow.ControlFlowMixin (Stage 2.4).
    ...
    # expose_proxy / forward_to / proxy / redirect — перенесены в
    # dsl.builders.integration.IntegrationMixin (Stage 2.5).
    ...
    # ── Entity CRUD / Audit / AV / HTTP / DB / file / S3 / timer / poll ──

    # entity_create / entity_get / entity_update / entity_delete / entity_list /
    # audit / scan_file / http_call / db_query / read_file / write_file /
    # read_s3 / write_s3 / timer / poll — перенесены в
    # dsl.builders.integration.IntegrationMixin (Stage 2.5).
    ...
    # db_query_external — перенесён в dsl.builders.integration.IntegrationMixin
    # (Stage 2.5).
    ...
    # notify — перенесён в dsl.builders.integration.IntegrationMixin (Stage 2.5).
    ...
    # file_move / shell / email — перенесены в dsl.builders.integration.IntegrationMixin
    # (Stage 2.5).
    ...
    # switch — перенесён в dsl.builders.control_flow.ControlFlowMixin (Stage 2.4).
    ...
    # on_error — перенесён в dsl.builders.control_flow.ControlFlowMixin (Stage 2.4).
    ...
    # перенесены в dsl.builders.integration.IntegrationMixin (Stage 2.5).
    ...
    # expire / correlation_id — перенесены в dsl.builders.control_flow.ControlFlowMixin
    # (Stage 2.4).
