...
async def root_page():
    ...
    def _safe_external_link(url: str | None) -> str | None:
        if not url:
            return None
        return utilities.ensure_url_protocol(url)

    processed_urls = {
        "log_url": _safe_external_link(settings.logging.base_url),
        "fs_url": _safe_external_link(settings.storage.interface_endpoint),
        "prefect_url": _safe_external_link(settings.app.prefect_url),
        "prefect_api_url": _safe_external_link(settings.app.prefect_api_url),
        "queue_url": _safe_external_link(settings.queue.queue_ui_url),
    }
        ...
        <div class="container">
            ...
            <div class="service-links">
                <h2>Основные интерфейсы</h2>
                    <a href="{processed_urls["log_url"]}" target="_blank" rel="noopener noreferrer">Мониторинг логов</a>
                    <a href="{processed_urls["fs_url"]}" target="_blank" rel="noopener noreferrer">Файловое хранилище</a>
                    <a href="{processed_urls["queue_url"]}" target="_blank" rel="noopener noreferrer">Управление очередями</a>
                    <a href="{processed_urls["prefect_url"]}" target="_blank" rel="noopener noreferrer">Оркестрация процессов</a>
            </div>

            <div class="documentation-links">
                <h2>Документация</h2>
                    <a href="/docs">REST API</a>
                    <a href="/asyncapi">AsyncAPI</a>
                    <a href="/redoc">ReDoc</a>
                    <a href="{processed_urls["prefect_api_url"]}" target="_blank" rel="noopener noreferrer">API Prefect</a>
            </div>
