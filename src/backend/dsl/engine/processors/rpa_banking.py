...
class AppiumMobileProcessor(BaseProcessor):
    ...
    def to_spec(self) -> dict[str, Any] | None:
        """Сериализовать конфигурацию процессора в dict (для YAML/JSON spec). Returns None для non-serializable state."""
        return {'appium_mobile': {'platform': self.platform, 'app_package': self.app_package, 'operation': self.operation}}
...
class EmailDrivenProcessor(BaseProcessor):
    ...
    def __init__(self, mailbox: str='INBOX', subject_filter: str | None=None, extract: str='body_table') -> None:
