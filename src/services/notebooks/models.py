"""Pydantic-модели Notebook и NotebookVersion (Wave 9.1).

Хранилище-агностичны: одинаковая форма для in-memory, MongoDB.
Версионирование — append-only: каждое обновление контента создаёт
новую запись в ``versions``; ``latest_version`` указывает на текущую.
"""
