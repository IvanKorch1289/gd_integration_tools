"""К5 (Wave K5/chaos-tests) — chaos для ``mq_chain``.
...
def test_mq_chain_chaos(scenario: str) -> None:
    """``mq`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.mq_chain"
    )
