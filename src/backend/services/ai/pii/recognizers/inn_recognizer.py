...
def _inn_checksum_valid(value: str) -> bool:
    ...
    if len(digits) not in (10, 12):
        return False
    # Отсеиваем тривиальные случаи (все одинаковые цифры)
    if len(set(digits)) == 1:
        return False
    if len(digits) == 10:
        checksum = sum(d * w for d, w in zip(digits[:9], _INN_10_WEIGHTS)) % 11 % 10
        return checksum == digits[9]
    # 12-значный ИНН
    cs1 = sum(d * w for d, w in zip(digits[:10], _INN_12_WEIGHTS_1)) % 11 % 10
    cs2 = sum(d * w for d, w in zip(digits[:11], _INN_12_WEIGHTS_2)) % 11 % 10
