...
def test_preview_count_clamped() -> None:
    result = validate_cron_expression("* * * * *", timezone="UTC", preview_count=999)
    assert result.is_valid
    assert len(result.next_executions) == 50


def test_preview_count_min_clamped_to_one() -> None:
    result = validate_cron_expression("* * * * *", timezone="UTC", preview_count=0)
    assert result.is_valid
    assert len(result.next_executions) == 1


def test_result_is_frozen_dataclass() -> None:
    result = validate_cron_expression("0 9 * * *", timezone="UTC", preview_count=1)
    with pytest.raises(Exception):
        result.is_valid = False  # type: ignore[misc]


def test_dataclass_fields() -> None:
    result = validate_cron_expression("0 9 * * *", timezone="UTC", preview_count=2)
