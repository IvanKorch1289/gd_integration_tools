"""router_specialist_processor переехал в :mod:`dsl.engine.processors.router_specialist_processor`.

W2 P0-3 Phase 1B (cycle 152, MINIMAX plan): canonical-реализация теперь живёт в
``src.backend.dsl.engine.processors.router_specialist_processor``. Этот модуль — re-export shim
...
``src.backend.dsl.processors.router_specialist_processor`` emits ``DeprecationWarning``.
...
_warnings.warn(
    "src.backend.dsl.processors.router_specialist_processor is deprecated; "
    "import from src.backend.dsl.engine.processors.router_specialist_processor instead. "
    "See ADR-0313/0314 (W2 P0-3 processor consolidation). Removal planned: cycle 156.",
    DeprecationWarning,
    stacklevel=2,
)

_CANONICAL_MODULE = "src.backend.dsl.engine.processors.router_specialist_processor"
