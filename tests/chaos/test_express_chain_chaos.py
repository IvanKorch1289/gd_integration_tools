"""К5 (Wave K5/chaos-tests) — chaos для ``express_chain``.
...
def test_express_chain_chaos(scenario: str) -> None:
    """``express`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.express_chain"
    )
