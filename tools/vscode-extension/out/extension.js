...
async function activate(context) {
    ...
    const serverAddress = config.get('lsp.server', 'localhost:8765');
    ...
    const clientOptions = {
        ...
        errorHandler: {
            ...
            closed: () => {
                return { action: node_1.CloseAction.Restart, message: 'LSP connection closed' };
            },
        },
    };
    ...
    client = new node_1.LanguageClient('gd-integration-tools-lsp', 'GD Integration Tools LSP', serverOptions, clientOptions);
...
function registerCommands(context) {
    // Run Step command
    const runStepCommand = vscode.commands.registerCommand('gd-integration-tools.runStep', async (uri) => {
        const document = uri
        ...
        if (client?.state === node_1.State.Running) {
            ...
            client.sendRequest('gd/runStep', params).then((result) => {
                if (result) {
    ...
    // Show Docs command
    const showDocsCommand = vscode.commands.registerCommand('gd-integration-tools.showDocs', async (uri) => {
        const document = uri
            ? await vscode.workspace.openTextDocument(uri)
            : vscode.window.activeTextEditor?.document;
        if (!document) {
        ...
        if (client?.state === node_1.State.Running) {
            ...
            client.sendRequest('gd/documentation', params).then((result) => {
                if (result) {
                    const panel = vscode.window.createWebviewPanel('gd-docs', 'GD Documentation', vscode.ViewColumn.Beside, {});
    ...
    const wizardRouteCommand = vscode.commands.registerCommand("gd-integration-tools.wizardRoute", async () => {
        ...
        const config = vscode.workspace.getConfiguration("gdIntegrationTools");
        const pythonCmd = config.get("pythonCommand", "uv run python");
    ...
    const wizardPluginCommand = vscode.commands.registerCommand("gd-integration-tools.wizardPlugin", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showWarningMessage("No workspace folder open");
            return;
    ...
    const validateRouteCommand = vscode.commands.registerCommand("gd-integration-tools.validateRoute", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            return;
        }
    ...
    const openRoutesCommand = vscode.commands.registerCommand("gd-integration-tools.openRoutesFolder", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            return;
        }
        const config = vscode.workspace.getConfiguration("gdIntegrationTools");
        const routesDir = config.get("routesDir", "routes");
        const folderUri = vscode.Uri.joinPath(workspaceFolder.uri, routesDir);
        await vscode.commands.executeCommand("revealFileInOS", folderUri);
    });
    // GD Open Extensions Folder
    const openExtensionsCommand = vscode.commands.registerCommand("gd-integration-tools.openExtensionsFolder", async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            return;
        }
        const config = vscode.workspace.getConfiguration("gdIntegrationTools");
        const extensionsDir = config.get("extensionsDir", "extensions");
        const folderUri = vscode.Uri.joinPath(workspaceFolder.uri, extensionsDir);
        await vscode.commands.executeCommand("revealFileInOS", folderUri);
    });
    context.subscriptions.push(runStepCommand, showDocsCommand, wizardRouteCommand, wizardPluginCommand, validateRouteCommand, openRoutesCommand, openExtensionsCommand);
}
/**
