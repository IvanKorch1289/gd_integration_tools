...
class TestSendEmailFromTemplateValidation:
    ...
    @pytest.mark.asyncio
    async def test_rejects_dotdot_in_template_name(self, mail_service: MailService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_accepts_simple_name(self, mail_service: MailService, tmp_path: Path) -> None:
        template_file = tmp_path / "hello.txt"
        template_file.write_text("Hello")

        with patch("aiofiles.open", AsyncMock()) as mock_open, patch.object(
            mail_service, "send_email", new_callable=AsyncMock
        ) as mock_send:
            mock_open.return_value.__aenter__.return_value.read = AsyncMock(return_value="Hello")
            await mail_service.send_email_from_template(
                to_emails=["a@example.com"],
                subject="subj",
