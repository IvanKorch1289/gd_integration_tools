...
class TestAdminListWorkflowTemplates:
    ...
    def test_returns_list_on_200(self, admin_client: Any) -> None:
    ...
    def test_returns_empty_on_non_list(self, admin_client: Any) -> None:
        """Non-list response (e.g. dict error) → return []."""
        with patch.object(admin_client, "_request", return_value={"error": "x"}):
            result = admin_client.list_workflow_templates()
        assert result == []

    def test_returns_empty_on_connection_error(self, admin_client: Any) -> None:
...
class TestWorkflowsListAllIds:
    ...
    def test_filters_empty_workflow_names(self, workflows_client: Any) -> None:
