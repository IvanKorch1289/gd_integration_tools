"""Credit namespace MCP tools (ADR-0070 §1, S27 W4).

Credit namespace содержит все actions с prefix ``credit.``.
...
def register_credit_tools(mcp: "FastMCP") -> None:
    """Регистрирует все credit.* actions как MCP tools в namespace.
    ...
    for action_name in credit_actions:
        _register_credit_tool(mcp, action_name)


def _register_credit_tool(mcp: "FastMCP", action_name: str) -> None:
    """Регистрирует один credit action как MCP tool."""
    import inspect
