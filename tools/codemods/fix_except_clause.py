...
.. warning::
    ADR-0304 (2026-09-21): НЕ применять репозиторий-широко. Форма
    ``except (A, B):`` НЕ канонична для ``ruff format`` при
    ``target-version = "py314"`` — форматтер снимает скобки обратно
    (PEP 758). Утилита оставлена для точечного применения.
