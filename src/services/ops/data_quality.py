...
class DataQualityMonitor:
    ...
    def __init__(self) -> None:
        ...
        self._stats: dict[str, dict[str, Any]] = defaultdict(lambda: {"checks": 0, "violations": 0})
    ...
    def list_rules(self) -> list[dict[str, Any]]:
        return [
            {"name": r.name, "field": r.field, "check": r.check,
             "severity": r.severity.value, "enabled": r.enabled}
            for r in self._rules
        ]

    async def check(self, data: dict[str, Any] | list[dict[str, Any]], dataset: str = "default") -> dict[str, Any]:
        ...
        return {
            ...
            "violations": [
                {"rule": v.rule, "field": v.field, "severity": v.severity.value,
                 "message": v.message, "value": v.value}
                for v in result.violations
            ],
        }

    async def schema_infer(self, data: dict[str, Any] | list[dict[str, Any]], dataset: str = "default") -> dict[str, Any]:
        ...
        self._inferred_schemas[dataset] = {k: v[0] if len(v) == 1 else str(v) for k, v in schema.items()}
        return {"schema": self._inferred_schemas[dataset], "drift": drift, "fields": len(inferred)}
    ...
    def _apply_rule(self, rule: DQRule, record: dict[str, Any], dataset: str) -> DQViolation | None:
        value = record.get(rule.field)

        if rule.check == "not_null":
            if value is None or value == "":
                return DQViolation(rule.name, rule.field, rule.severity,
                                   f"Field '{rule.field}' is null/empty", value)
            ...
            if isinstance(value, (int, float)):
                ...
                if min_val is not None and value < min_val:
                    return DQViolation(rule.name, rule.field, rule.severity,
                                       f"Value {value} < min {min_val}", value)
                if max_val is not None and value > max_val:
                    return DQViolation(rule.name, rule.field, rule.severity,
                                       f"Value {value} > max {max_val}", value)
            ...
            if str_val in self._seen_keys[key]:
                return DQViolation(rule.name, rule.field, rule.severity,
                                   f"Duplicate value: {str_val[:50]}", value)
            ...
            if isinstance(value, (int, float)):
                ...
                if len(history) >= 10:
                    ...
                    if stddev > 0:
                        ...
                        if z > threshold:
                            return DQViolation(rule.name, rule.field, DQSeverity.WARNING,
                                               f"Outlier: z-score={z:.2f}", value)
