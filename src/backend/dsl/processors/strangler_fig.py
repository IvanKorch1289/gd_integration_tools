"""strangler_fig переехал в :mod:`dsl.engine.processors.strangler_fig`.

W2 P0-3 Phase 1B (cycle 152, MINIMAX plan): canonical-реализация теперь живёт в
``src.backend.dsl.engine.processors.strangler_fig``. Этот модуль — re-export shim
...
``src.backend.dsl.processors.strangler_fig`` emits ``DeprecationWarning``.
...
_warnings.warn(
    "src.backend.dsl.processors.strangler_fig is deprecated; "
    "import from src.backend.dsl.engine.processors.strangler_fig instead. "
    "See ADR-0313/0314 (W2 P0-3 processor consolidation). Removal planned: cycle 156.",
    DeprecationWarning,
    stacklevel=2,
)

_CANONICAL_MODULE = "src.backend.dsl.engine.processors.strangler_fig"
