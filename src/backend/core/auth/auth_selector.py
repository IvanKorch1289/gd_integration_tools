"""S96 W1 — Auth Selector: реализация verification логики в core-слое.

Архитектура (S95 W4 → S96 W1):

* ``core.auth.auth_selector`` — implementation (verifier registry,
  :func:`verify_request`, :func:`require_auth` factory). Stable import
  path для extensions.
* ``core.auth.gateway`` — public facade (re-exports top-level API).
* ``entrypoints.api.dependencies.auth_selector`` — deprecated shim,
  re-exports из ``core.auth.auth_selector`` (для backward compat).
  Удалится в S99+.

S93 W3 уже экспонировал public ``verify_request`` в entrypoints; S95 W4
создал ``core.auth.gateway`` фасад; S96 W1 — переносит РЕАЛЬНУЮ
implementation в core (избежание downward layer violation:
``core/ → entrypoints/``).
...
async def _verify_express_jwt(request: Request) -> AuthContext | None:
    ...
    Проверяет ``Authorization: Bearer *** где токен подписан
...
async def verify_request(
    ...
    """Public single entry-point для verify-flow (S93 W3, S96 W1 relocate).

    Раньше middleware (auth_required) лез в private ``_VERIFIERS`` dict
    напрямую — нарушение инкапсуляции. S93 W3 экспонировал public API.
    S96 W1 — перенёс implementation в ``core/auth/auth_selector.py``
    (избежание downward layer violation).
