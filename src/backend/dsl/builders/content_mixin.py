...
class EnrichEIPProcessor(BaseProcessor):
    ...
    def _fetch(self, exchange: "Exchange[Any]") -> Any:
        if self.strategy == "http":
            ...
            with urllib.request.urlopen(_resolve(self.source, exchange), timeout=5) as r:  # noqa: S310
...
class WireTapEIPProcessor(BaseProcessor):
    ...
    def __init__(self, *, sink: str, async_: bool = True, name: str | None = None) -> None:
    ...
    async def process(
        ...
        if self.async_:
            ...
            return None  # sink resolved downstream
