...
class TestAiGatewayFacadeAllExports:
    ...
    @pytest.mark.parametrize(
        "symbol_name", ["AIGateway", "AIRequest", "AIResponse", "EnforcedInvokeMixin"]
    )
    def test_all_exports_accessible(self, symbol_name: str) -> None:
        """Каждый символ из ``__all__`` доступен через facade."""
        assert hasattr(gateway, symbol_name), f"Missing export: {symbol_name}"
        assert symbol_name in gateway.__all__, f"{symbol_name} not declared in __all__"
