...
class TestOtelMiddleware:
    ...
    async def test_no_tracer_passes_through(
        ...
        send = AsyncMock()
        await middleware_no_tracer(_make_scope("GET", "/path"), _make_receive(), send)
    ...
    async def test_5xx_response_marks_error(
        ...
        all_calls_str = (
            " ".join(str(call) for call in span_mock.set_attribute.call_args_list)
            + " "
            + " ".join(str(call) for call in span_mock.set_status.call_args_list)
        )
    ...
    def test_inject_traceparent(self, middleware_with_tracer: OtelMiddleware) -> None:
        ...
        assert any(v == b"00-abc-123-01" for k, v in headers if k == b"traceparent")
...
class TestOtelMiddlewarePureASGI:
    ...
    async def test_traceparent_in_response_headers(
        ...
        middleware_with_tracer._propagator.inject.side_effect = lambda carrier: (
            carrier.update({"traceparent": "00-trace-123-01"})
        )
