...
6. **Внешние аудиты на Python < 3.14 дают ложные «блокеры компиляции».**
   Аудит-клоны с системным Python 3.12/3.13 сообщают «N модулей не
   компилируются» из-за `except A, B:` — это валидный PEP 758 на целевом
   3.14 (ADR-0304, каноническая форма). Любой статический анализ прогонять
