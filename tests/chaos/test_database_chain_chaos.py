"""К5 (Wave K5/chaos-tests) — chaos для ``database_chain``.
...
def test_database_chain_chaos(scenario: str) -> None:
    """``database`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.database_chain"
    )
