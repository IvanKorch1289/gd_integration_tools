# Privacy Redis adapter audit — cycle 158+ continuation (2026-09-24)
...
- 4 erasure adapter classes EXIST (Redis/S3/Qdrant/Langmem).
- Per `check_privacy_lifecycle.py` gate: 4 reported as incomplete.
- Actual: per-adapter audit needed (NOT estimates).

This document: audit of `RedisErasureAdapter` per audit "Всегда перепроверяй".

## 2. Source verification (`src/backend/core/privacy/delete_data_subject/_redis.py`)
...
- **SCAN + UNLINK** для subject-related cache keys.
- **Pattern matching**: `match=f"*{prefix}{subject_id}*"` (e.g., `*user:42*`).
- **Multi-prefix support**: 6 default prefixes (`user:`, `session:`, `tenant:`, etc.).

### 2.3 Per audit "Всегда перепроверяй" findings

**✅ REAL implementation exists** (NOT stub):
- Uses `redis.asyncio.Redis.scan` (async iteration with cursor).
- Uses `redis.asyncio.Redis.unlink` (non-blocking delete).
- Returns `AdapterResult` with `erased_keys` count.
...
- ❌ **Pipeline wiring**: `RedisErasureAdapter` is exported from `__init__.py`, BUT actual wiring in composition root needs verification.
...
| Adapter uses `current_tenant()` from TenantContext | ❌ **No** (per source code inspection) |
| Adapter verifies caller.tenant_id matches resource.tenant_id | ❌ **No** |
| Adapter uses `tenant_id` in SCAN pattern | ❌ **No** (uses `subject_id` only) |
...
Per cycle 158+ investigation + this Redis audit:
...
3. **Pipeline wiring needs audit** — is `RedisErasureAdapter` actually called from
...
Redis fix scope:
- ❓ **Audit pipeline wiring**: is RedisErasureAdapter called from DeleteDataSubject?
...
| Architecture fit | ✅ Matches postgresql pattern (per cycle 158+ investigation) |
| Value | Realistic — Redis is per-session cache (highest user-data exposure) |
...
- Redis fix deferred to cycle 159+ (out of current session scope).

## 8. Per audit goal audit (Privacy backends status — Redis specifically)

| Criterion | Status |
|---|---|
| Adapter EXISTS | ✅ **YES** (RedisErasureAdapter class) |
...
- `src/backend/core/privacy/delete_data_subject/_redis.py` (per-adapter source).
- `src/backend/core/privacy/delete_data_subject/_orchestrator.py` (DeleteDataSubject pipeline).
