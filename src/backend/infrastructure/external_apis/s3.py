...
class S3Service:
    ...
    def __init__(self, client: BaseS3Client):
        ...
        self._cache_handlers = {'metadata': metadata_cache, 'existence': existence_cache}

    async def upload_file(self, key: str, content: bytes, original_filename: str, content_type: str | None=None, extra_metadata: dict[str, str] | None=None) -> dict[str, Any]:
        """Выполнить операцию upload file."""
        metadata: dict[str, str] = {'original-filename': original_filename}
        if content_type:
            metadata['content-type'] = content_type
        if extra_metadata:
            metadata.update({str(meta_key): str(meta_value) for meta_key, meta_value in extra_metadata.items() if meta_value is not None})
        encoded_metadata = encode_base64(metadata)
        result = await self.client.put_object(key=key, body=content, metadata=encoded_metadata)
        ...
        async def stream_generator():
            ...
            async with body as stream:
                while (chunk := (await stream.read(64 * 1024))):
                    yield chunk
        return StreamingResponse(stream_generator(), media_type=content_type, headers={'Content-Disposition': f'attachment; filename="{filename}"'})
    ...
    async def create_zip_archive(self, keys: list[str]) -> StreamingResponse:
        ...
        buffer.seek(0)
        return StreamingResponse(iter([buffer.getvalue()]), media_type='application/zip', headers={'Content-Disposition': 'attachment; filename="archive.zip"'})
