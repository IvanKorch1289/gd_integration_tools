...
class PluginTenantDecl(BaseModel):
    ...
    Используется :class:`PluginLoader` для вызова
    ...
    def _validate_grammar(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        ...
        :attr:`PluginManifest.capabilities`.
        """
...
class PluginManifest(BaseModel):
    ...
    * ``"B"`` — untrusted/external (default): strict e2b/pyodide
    ...
    регистрирует для конкретного tenant'а. :class:`PluginLoader`
    ...
    (см. :meth:`PluginLoader._load_one`).
    """
    ...
    def _validate_tenants_against_catalog(self) -> PluginManifest:
...
def load_plugin_manifest(path: Path | str) -> PluginManifest:
    ...
    Returns:
        Валидированный :class:`PluginManifest`.
