...
async def _emit_audit(event: str, processor: str, params: dict[str, Any], result: dict[str, Any] | None=None, error: str | None=None) -> None:
    ...
    try:
        ...
        if audit is None:
            ...
            return
        outcome: Literal['success', 'failure', 'denied'] = 'failure' if error else 'success'
        severity: str = 'warning' if error else 'info'
        payload = {'event': event, 'actor': f'processor:{processor}', 'resource': f'banking/{processor}', 'action': event.split('.')[-1] if '.' in event else event, 'outcome': outcome, 'severity': severity, 'details': {'processor': processor, 'params': params, 'result': result, 'error': error}}
...
class _BankingAIProcessor(BaseProcessor):
    ...
    async def _call_llm(self, prompt: str, output_model: type[_T], exchange: Exchange[Any], context: ExecutionContext, model: str | None=None) -> _T | None:
        ...
        async def _chat_with_retry() -> Any:
            return await agent.chat(messages=[{'role': 'user', 'content': prompt}], model=model or 'default', response_format=output_model)
        try:
...
class KycAmlVerifyProcessor(_BankingAIProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        if not await self._check_capability(exchange, context):
            return
        result = await self._call_llm(prompt=prompt, output_model=KycAmlResult, exchange=exchange, context=context, model=self.model)
        if result is None:
            await _emit_audit(event=f'{self.audit_event_prefix}.failed', processor=self.name, params={'jurisdiction': self.jurisdiction}, error='llm_call_failed')
            return
        await _emit_audit(event=f'{self.audit_event_prefix}.completed', processor=self.name, params={'jurisdiction': self.jurisdiction, 'customer_id': customer.get('id')}, result={'decision': result.decision, 'score': result.score, 'reasons': result.reasons})
        exchange.in_message.set_body({'decision': result.decision, 'score': result.score, 'reasons': result.reasons})
    ...
    async def _check_capability(self, exchange: Exchange[Any], context: ExecutionContext) -> bool:
        ...
        try:
            ...
            await _emit_audit(event=f'{self.audit_event_prefix}.capability_denied', processor=self.name, params={}, error=str(exc))
            return False
...
class AntiFraudScoreProcessor(_BankingAIProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        if not await self._check_capability(exchange, context):
            return
        result = await self._call_llm(prompt=prompt, output_model=AntiFraudResult, exchange=exchange, context=context, model=self.model)
        if result is None:
            await _emit_audit(event=f'{self.audit_event_prefix}.failed', processor=self.name, params={}, error='llm_call_failed')
            return
        await _emit_audit(event=f'{self.audit_event_prefix}.completed', processor=self.name, params={'transaction_id': transaction.get('id')}, result={'risk_score': result.risk_score, 'risk_level': result.risk_level, 'triggered_rules': result.triggered_rules})
        exchange.in_message.set_body({'risk_score': result.risk_score, 'risk_level': result.risk_level, 'explanation': result.explanation, 'triggered_rules': result.triggered_rules})
        exchange.set_property('fraud_risk_score', result.risk_score)
        exchange.set_property('fraud_risk_level', result.risk_level)

    def _build_prompt(self, transaction: dict[str, Any], history: list[dict[str, Any]], rules_triggered: list[str]) -> str:
...
class CreditScoringRagProcessor(_BankingAIProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        if not await self._check_capability(exchange, context):
            return
        result = await self._call_llm(prompt=prompt, output_model=CreditScoringResult, exchange=exchange, context=context)
        if result is None:
            await _emit_audit(event=f'{self.audit_event_prefix}.failed', processor=self.name, params={'product': product}, error='llm_call_failed')
            return
        await _emit_audit(event=f'{self.audit_event_prefix}.completed', processor=self.name, params={'customer_id': customer.get('id'), 'amount': amount, 'product': product}, result={'approved': result.approved, 'limit': result.limit, 'rate': result.rate, 'citations': result.citations})
        exchange.in_message.set_body({'approved': result.approved, 'limit': result.limit, 'rate': result.rate, 'citations': result.citations, 'reasons': result.reasons})
    ...
    def _format_rag_context(self, rag_context: Any) -> str:
        ...
        if isinstance(rag_context, list):
            return '\n---\n'.join((item.get('document', str(item)) if isinstance(item, dict) else str(item) for item in rag_context))
        return str(rag_context)

    def _build_prompt(self, customer: dict[str, Any], amount: float, product: str, rag_context: str) -> str:
...
class DocumentClassifierProcessor(_BankingAIProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        if not await self._check_capability(exchange, context):
            return
        result = await self._call_llm(prompt=prompt, output_model=DocumentClassifierResult, exchange=exchange, context=context, model=self.model)
        if result is None:
            await _emit_audit(event=f'{self.audit_event_prefix}.failed', processor=self.name, params={'type_hint': type_hint}, error='llm_call_failed')
            return
        await _emit_audit(event=f'{self.audit_event_prefix}.completed', processor=self.name, params={'type_hint': type_hint}, result={'document_type': result.document_type, 'confidence': result.confidence, 'subtypes': result.subtypes})
        exchange.in_message.set_body({'document_type': result.document_type, 'confidence': result.confidence, 'subtypes': result.subtypes, 'extracted_fields': result.extracted_fields})
...
class FrancotypingProcessor(_BankingAIProcessor):
    ...
    async def process(self, exchange: Exchange[Any], context: ExecutionContext) -> None:
        ...
        if not await self._check_capability(exchange, context):
            return
        result = await self._call_llm(prompt=prompt, output_model=FrancotypingResult, exchange=exchange, context=context, model=self.model)
        if result is None:
            await _emit_audit(event=f'{self.audit_event_prefix}.failed', processor=self.name, params={}, error='llm_call_failed')
            return
        await _emit_audit(event=f'{self.audit_event_prefix}.completed', processor=self.name, params={'text_length': len(text)}, result={'language': result.language, 'region': result.region, 'script': result.script, 'confidence': result.confidence})
        exchange.in_message.set_body({'language': result.language, 'region': result.region, 'script': result.script, 'classification': result.classification, 'confidence': result.confidence})
