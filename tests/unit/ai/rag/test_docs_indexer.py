...
1. __init__ / defaults / DI / overlap-clamp
2. discover_docs (default roots, custom roots, recursive, .md filter)
3. chunk_text (basic, empty, overlap, metadata, line numbers)
4. index_docs (returns N, idempotent, fallback, embedder DI)
5. search (results, empty query → ValueError, post-index hit)
"""
...
from src.backend.ai.rag.docs_indexer import (
    _DEFAULT_ROOTS,
    DocsIndexer,
...
def test_indexer_custom_init() -> None:
    """Init с кастомными параметрами (DI: qdrant_client, model, size)."""
    ...
    assert idx._embedding_model == "text-embedding-3-large"
...
def test_indexer_default_roots() -> None:
    """Default roots = CLAUDE.md, .claude/, docs/users, docs/devs (per v15 §10 W5)."""
...
def test_indexer_discover_finds_md(tmp_path: Path) -> None:
    """discover_docs рекурсивно ищет .md файлы во вложенных директориях."""
...
def test_indexer_discover_ignores_non_md(tmp_path: Path) -> None:
    """discover_docs игнорирует .py, .json, .txt — только .md."""
...
def test_chunk_text_basic() -> None:
    """text → N chunks базового размера (25 chars / 10 step = 3 chunks)."""
    idx = DocsIndexer(chunk_size=10, chunk_overlap=0)
    chunks = idx.chunk_text("a" * 25, {"file": "x.md"})
    assert len(chunks) == 3
    assert all(c["text"] for c in chunks)


def test_chunk_text_empty() -> None:
    """empty/whitespace text → [] (нет chunks, без падений)."""
    idx = DocsIndexer()
...
async def test_index_docs_returns_count(tmp_path: Path) -> None:
    """index_docs возвращает N chunks (300 chars / 100 = 3, 50 chars / 100 = 1)."""
    a = tmp_path / "a.md"
    b = tmp_path / "b.md"
...
async def test_index_docs_no_docs_returns_zero() -> None:
    """index_docs с пустым списком → 0 (без падений, no collection creation)."""
    idx = DocsIndexer()
...
async def test_search_returns_results() -> None:
    ...
    if results:  # может быть пусто если hash-based embedder не даёт матч
        for r in results:
            assert "id" in r
            assert "score" in r
...
async def test_search_after_index_finds_chunk() -> None:
    """После index_docs, search(query) находит ≥1 chunk (offline embedder deterministic)."""
    with tempfile.TemporaryDirectory() as d:
        p = Path(d) / "guide.md"
...
# ===== fallback / graceful degradation / embedder DI =====
...
async def test_indexer_fallback_upsert_and_search() -> None:
    """In-memory fallback: upsert → search → возвращает hits, ordered by cosine."""
...
def test_indexer_set_embedder_uses_provided() -> None:
    """set_embedder DI: переданный embed_fn используется; chainable returns self."""
    idx = DocsIndexer()
    calls: list[list[str]] = []
    ...
    ret = idx.set_embedder(my_embed)
    assert ret is idx  # chainable
