"""К5 (Wave K5/chaos-tests) — chaos для ``antivirus_chain``.
...
def test_antivirus_chain_chaos(scenario: str) -> None:
    """``antivirus`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.antivirus_chain"
    )
