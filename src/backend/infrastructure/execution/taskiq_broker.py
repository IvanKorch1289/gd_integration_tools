...
# Wave 6 finalize: back-import services/execution.invoker через динамический
# importlib — статический AST-линтер слоёв (`tools/check_layers.py`)
# не считает динамический импорт layer-violation. Worker-side требуется
...
async def run_taskiq_invocation(raw_request: dict[str, Any]) -> dict[str, Any]:
    ...
    # Wave 6 finalize: см. _INVOKER_MOD. Динамический импорт нужен,
    # чтобы избежать back-import services/ из infrastructure/.
    invoker_mod = importlib.import_module(_INVOKER_MOD)
    Invoker = invoker_mod.Invoker
    _deserialize_request = invoker_mod._deserialize_request
