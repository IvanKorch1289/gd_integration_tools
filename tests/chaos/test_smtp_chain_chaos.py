"""К5 (Wave K5/chaos-tests) — chaos для ``smtp_chain``.
...
def test_smtp_chain_chaos(scenario: str) -> None:
    """``smtp`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.smtp_chain"
    )
