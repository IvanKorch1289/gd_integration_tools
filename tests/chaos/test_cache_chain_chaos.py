"""К5 (Wave K5/chaos-tests) — chaos для ``cache_chain``.
...
def test_cache_chain_chaos(scenario: str) -> None:
    """``cache`` chain выживает scenario={scenario} через resilience-coordinator."""
    # Smoke: проверяем импорт chain'а и помечаем сценарий.
    assert_chain_module_loadable(
        "src.backend.infrastructure.resilience.components.cache_chain"
    )
