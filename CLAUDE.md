...
3. Если задача попадает под триггеры внешнего исследования — сначала выполнить web research (DuckDuckGo MCP + Fetch MCP / WebSearch / WebFetch), затем составлять план или давать рекомендацию.
...
Только если задача требует актуальных внешних данных, использовать MCP web search (DuckDuckGo) + Fetch MCP / WebSearch / WebFetch.

### Когда внешний поиск обязателен

Перед ответом ОБЯЗАТЕЛЬНО использовать web search (DuckDuckGo MCP), если вопрос относится хотя бы к одному из классов:
...
### Правила использования web search

1. Сначала сформулировать 2–5 коротких целевых запросов для DuckDuckGo MCP.
2. Сначала проверять:
   - официальную документацию;
   - GitHub releases / changelog / issues;
   - PyPI;
   - официальные migration guides.
3. Использовать Fetch MCP для углублённого чтения 1-2 наиболее релевантных страниц.
4. Не отвечать на вопросы о совместимости, сравнении и новых библиотеках без внешней проверки, если вопрос явно зависит от актуального состояния экосистемы.
5. В итоговом ответе разделять:
   - внешние подтверждённые факты (со ссылкой и датой);
   - вывод применительно к архитектуре этого проекта.
6. Если web search недоступен, явно сообщить, что ответ будет без актуальной внешней проверки.
