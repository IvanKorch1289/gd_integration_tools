...
class JwtBlacklistProtocol(Protocol):
    ...
    async def is_revoked(self, jti: str) -> bool:
        """True если JWT ``jti`` (token id) revoked."""
        ...

    async def revoke(self, jti: str, expires_at: int) -> None:
        """Revoke JWT ``jti`` до ``expires_at`` (token остаётся в blacklist до TTL)."""
        ...

    async def revoke_before_time(self, time_threshold: int) -> None:
        """Bulk-revoke все tokens с iat < ``time_threshold`` (logout-all)."""
        ...

    async def is_iat_revoked(self, iat: int | None) -> bool:
        """True если issued-at (``iat``) находится в blacklist (для issued-before-revoke)."""
        ...
