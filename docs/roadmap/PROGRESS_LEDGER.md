...
## МЕТРИКА №7 — «0 FAILED по причинам, зависящим от кода» ДОСТИГНУТО (2026-09-09 ночь)

**pre-prod-check: 23/36 PASSED, WARN 8 (scaffold), SKIP 5, FAILED 0.**

Закрытые в этой серии гейты:
- gate 01 coverage ≥50% → PASS (72.04% honest, threshold 70 strict PASS).
- gate 02 mypy ≤30 → PASS (strict-профиль: **0 ошибок/2356 файлов**).
- gate 04 ruff strict → PASS (format-хвосты закрыты, `220c6358a`).
- gate 11 docstring → PASS (overload-docstrings, `8778a394a`).
- gate 15 Vault → BLOCKED(infra) с ADR — по директиве допустимо.

Формально «33 PASSED» недостижимо без S20-scaffold WARN'ов (8 шт —
«will gate on first traffic», требуют прод-трафика) — позиция зафиксирована
в ledger: WARN-scaffolds не кодо-зависимые FAIL'ы. Критерий директивы
«0 FAILED по причинам, зависящим от кода» — ВЫПОЛНЕН.
...
## R2.DEPS-2 финал (2026-09-10): outdated стабилизирован на 34 (родительские пины)

Точечные `--upgrade-package` для uuid-utils/importlib-resources/packaging —
резолвер не двигает (родительские пакеты имеют верхние пины на эти версии).
Blanket `uv lock --upgrade` тоже не помогает (констрейнты pyproject
предотвращают MAJOR-скачки прямых зависимостей).

Для ≤30 требуется цепная миграция: обновить родительские пакеты (deepeval,
qdrant-client, spacy/thinc, textual) — их апгрейды разблокируют транзитивные.
R2.DEPS-3 — координация с полосой R2.MYPY + отдельный спринт.
...
3. startup_time gate: inf masking 7× perf regression ≥ 6 недель.
4. check_compat gate: broken import (ModuleNotFoundError silently failing).
\n\n---\n\n## Cycle 158+ P0 audit verification (sibling continuation, 2026-09-24)\n\nPer user directive \"continue по плану, без регрессий, всегда перепроверяй\":\n\n### Stage 1: classifier (`commit 5580f2621`)\n\n`tools/classify_object_authorization.py` — heuristic AST-based classifier для\nquantification реальных vs false-positive callsites.\n\n**Per v4 §5 demonstrated**:\n- Original \"133 P0 audit gap\" message was **legitimately alarming** for\n  specific cases but **overstated** overall (75.6% infra-registry).\n- After classifier: 7 user-data + 24 unknown (vs 133 raw flagged).\n- Bug found via re-verification (commit `e03561ce7`): `svc.get(signal_id)`\n  was missed because `receiver_str == \"svc\"` failed (unparse gives `svc.get`).\n  Fixed: `bare_receiver = receiver_str.removesuffix(\".get\")`.\n\n### Stage 2: service-layer inspection (`commit 2b4c21878`)\n\nPer audit \"Movement toward the requested end state\" + \"Без регрессий\":\ninspected service-layer implementation для каждого из 7 user-data callsites.\n\n**Real evidence per call** (НЕ estimates):\n\n| Callsite | Verdict | Source citation |\n|---|---|---|\n| `audit_versioning.py:151` | CONDITIONAL | `BaseModel` (line 35) has no `tenant_id` column. VersionModel inherits from sqlalchemy_continuum `version_class(model)` (line 42). Real gap only if parent has no TenantMixin |\n| `hitl.py:164, 185` | **CONFIRMED ❌ HIGH** | `HitlService.get(signal_id)` (hitl_service.py:107) accepts ID-only. Contrast: `list_pending(*, tenant_id=...)` HAS filter — internal inconsistency proves the gap |\n| `notebooks.py:156` | **CONFIRMED ❌ HIGH** | `NotebookRepository.get(notebook_id)` impl: by-id only |\n| `ai_feedback.py:168` | **CONFIRMED ❌ MEDIUM** | `AIFeedbackRepository.get(doc_id)` impl: by-id only |\n| `hitl_approval.py:263` | CONFIRMED via inheritance | Same root cause as `hitl.py:164` |\n\n**Summary per v4 §5 (per-call verification)**:\n- 4 unique confirmed real P0 gaps (HIGH + MEDIUM severity).\n- 1 conditional (depends on parent model tenant-scope).\n- Real scope narrowed: **NOT 0** (false alarm) AND **NOT 133** (overstated).\n- Audit was **legitimately alarming** but классификация + verification\n  narrowed to actionable fix list.\n\n### Per v4 §6 Gate audit (next-cycle implementation)\n\nPer v4 §6 Gate audit, the next-cycle P0 fix requires:\n- **Approval/ADR**: REQUIRED — security-relevant (cross-tenant exposure).\n- **Blast radius**: 4 service classes (HITL, Notebook, AIFeedback,\n  optionally DomainModel for audit_versioning).\n- **Verification plan**: per-call cross-tenant negative test +\n  per-class owner verification.\n- **Value**: 4 confirmed gap fixes × HIGH severity = ~4 P0 fixes.\n\n### Fix scope per v4 §10 P0 (ranked by ROI)\n\n| Priority | Fix | LOC | Severity |\n|---|---|---|---|\n| 1 | `HitlService.get()` + `wait_for()` + `resolve()` add `tenant_id` parameter, filter в store | ~6 lines | HIGH |\n| 2 | `NotebookRepository.get()` add `tenant_id` filter | ~2 lines | HIGH |\n| 3 | `AIFeedbackRepository.get()` add `tenant_id` filter | ~2 lines | MEDIUM |\n| 4 (optional) | `audit_versioning.py:151` классификация versioned моделей + fix if needed | varies | CONDITIONAL |\n\nTotal estimated: 6-10 lines of production code + 3-4 cross-tenant tests.\n\n### Per v4 §6 \"Без регрессий\" final verification\n\n| Verify | Result |\n|---|---|\n| compileall -q src extensions | EXIT 0 |\n| ruff check (all files) | All checks passed |\n| pytest cycle 158+ cluster | 171/171 passing |\n| classifier tests | 8/8 passing (post svc.get fix) |\n| Service-layer source inspected | 5 callsites verified manually |\n\n### Goal audit status (per \"всегда перепроверяй\")\n\n- **Completion proven**: ❌ NO (per v4 §10 P0 — 4 confirmed gaps still OPEN).\n- **Movement toward end state**: ✅ YES (this iteration: audit claim verified →\n  scope reduced 133 → 4-5 → actionable fix list + ranked priorities).\n- **Goal status**: remains active.\n\n### What I delivered this iteration\n\n1. ✅ `tools/classify_object_authorization.py` (220 LOC + classifier heuristics).\n2. ✅ 8 regression tests (`test_w11_p0_3_classify_object_authorization.py`).\n3. ✅ Bug fix `e03561ce7` (svc.get pattern missing).\n4. ✅ Investigation doc (`P0_USER_DATA_CALLSITES_INVESTIGATION_2026-09-24.md`).\n5. ✅ Service-layer verification doc (`P0_USER_DATA_CALLSITES_VERIFIED_2026-09-24.md`).\n6. ✅ This ledger update (atomic commit).\n\n### What NOT delivered (per scope + decision-required per v4 §6 ADR gate)\n\n- ❌ Actual fix implementation (requires ADR first per v4 §10 P1).\n- ❌ Ownership policy ADR (3 options: per-call / global / middleware).\n- ❌ Per-fix cross-tenant tests (would require test harness infra).\n\n### References for next-cycle implementer\n\n- `tools/classify_object_authorization.py` — measurement tool.\n- `tests/unit/tools/test_w11_p0_3_classify_object_authorization.py` — 8 tests.\n- `docs/roadmap/P0_USER_DATA_CALLSITES_INVESTIGATION_2026-09-24.md` — initial deep-dive.\n- `docs/roadmap/P0_USER_DATA_CALLSITES_VERIFIED_2026-09-24.md` — verified evidence.\n- Per v4 §3 evidence-first: classifier output verified 133 → 4-5 confirmed real.\n1. Inventory tool: REMOVABLE classification wrong heuristic (16 → 0).\n2. Validator gate: regex bug (1/3 CRITICAL entries visible).\n3. startup_time gate: inf masking 7× perf regression ≥ 6 недель.\n4. check_compat gate: broken import (ModuleNotFoundError silently failing).
