"""Canonical ``AuditService`` — canonical home (S113 W1, завершает S103 W3).

Ранее жил в :mod:`src.backend.services.audit.audit_service` (192 LOC, тонкий
фасад над :class:`ClickHouseAuditService`). После S103 W3 split'а ``core/audit/facade``
на 6 доменов (waf/ai/capability/authorization/secrets/banking) канонический
домен ``AuditService`` оставался в ``services/`` — это создавало
``core/audit/facade → services/audit`` layer violation (2 violations:
``__init__.py`` + ``_base.py``).

S113 W1: переносим класс сюда, в ``core/audit/facade/`` (канонический домен
по ADR-0190 + S103 W3 design). ``services/audit/audit_service.py`` остаётся
как thin backward-compat re-export — не ломает 14 существующих consumers.
...
_logger = get_logger("core.audit.facade.unified")
