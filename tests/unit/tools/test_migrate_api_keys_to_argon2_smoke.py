...
class TestMigrationDryRunSafe:
    ...
    @pytest.mark.asyncio
    async def test_legacy_to_argon2_path_dry_run_no_set(self, tmp_path: Path) -> None:
        ...
        conn = MagicMock()
        stored = json.dumps(
            {"client_id": "c_dry", "key_hash": legacy_sha, "hash_algo": "sha256"}
        ).encode()
...
class TestMigrationMaxFailures:
    ...
    async def test_max_failures_aborts_loop(self) -> None:
        ...
        async def _get(key: str) -> bytes:
            idx = call_idx["i"]
            call_idx["i"] += 1
            return json.dumps({"client_id": f"c{idx}", "key_hash": legacy_sha}).encode()
        ...
        for redis_key in redis_keys:
            await mig._process_one(conn, redis_key, keys_map, dry_run=True, stats=stats)
