...
- **Encryption-per-tenant** — **удаляется** из scope V22 (`infrastructure/security/tenant_encryption.py` — wired-but-unused; revert-path inline в этом ADR — linear-cost migration добавит encryption layer поверх существующего TenantContext);
...
**Контекст**. Baseline-режим (continue-on-error: true) не блокирует merge при обнаружении CVEs. Текущий скан `uv run pip-audit -f json .` показывает 2 пакета с уязвимостями:
