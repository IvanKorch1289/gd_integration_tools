...
S44 W3: ``verify_and_dispatch`` обёрнут ``@require_capability("webhook.read")``.
В тестах мокаем ``get_authorization_facade`` чтобы capability check
разрешал (HMAC остаётся фактической аутентификацией webhook).
"""
...
def _allow_capability_mock() -> AsyncMock:
    """AsyncMock для AuthorizationFacade.check_principal → allow.

    Используется для тестов webhook: capability check должен быть
    mockable чтобы HMAC-валидация (реальная auth) могла выполниться.
    """
    mock_decision = MagicMock()
    mock_decision.allowed = True
    mock_decision.reason = None
    return AsyncMock(return_value=mock_decision)
...
async def test_canonical_mode_accepts_valid_signature() -> None:
    ...
    mock_facade.check_principal = _allow_capability_mock()
    with patch(
...
async def test_canonical_mode_rejects_expired_timestamp() -> None:
    ...
    with patch(
        ...
        with pytest.raises(WebhookVerificationError):
            await source.verify_and_dispatch(
                body,
                {"X-Signature": sig, "X-Timestamp": str(old_ts)},
                _principal="webhook-service",
            )
